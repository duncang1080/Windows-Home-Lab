...
Opening the JSON file, there are messages giving the status of the TPM (Trusted Platform Module) and see if it is failing or not. 
It tells me that the system is recognizing the TPM and communicating with it. The issue is that EK certificate is not available. 
